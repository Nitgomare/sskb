...
主要功能：
1. 按二级标题识别主章节，例如“## 1 Introduction”。
2. 对缺少章号、但后面出现“### 5.1 ...”的标题自动推断章号。
3. 将章标题提升为一级标题，但标题文字完全沿用原文，不自动添加章号。
4. 将每章引用的图片复制到该章自己的 images 文件夹，并重写链接。
5. 直接在图书项目根目录生成 mkdocs.yml，不再只生成 nav 片段。
6. 生成图书首页 docs/index.md、MathJax 配置和拆分报告。
7. 左侧只显示章节导航，不把页面内二、三级标题集成到左侧。
8. 不删除源 Markdown 和源图片；--clean 只清空目标项目的 docs 文件夹。
"""
...
SECTION_1_RE = re.compile(r"^\s*###\s+(\d{1,2})\.1(?:\s+|$)")
...
def find_chapter_starts(lines: list[str]) -> list[ChapterStart]:
    ...
    for index, line in enumerate(lines):
        ...
        cleaned = clean_heading_text(original)
        numbered = re.match(r"^(\d{1,2})\s+(.+)$", cleaned)

        if numbered:
            number = int(numbered.group(1))
            title = clean_heading_text(numbered.group(2))

            if 1 <= number <= 99 and number not in used_numbers:
                starts.append(
                    ChapterStart(
                        line_index=index,
                        number=number,
    ...
    if not starts:
        raise ValueError(
            "没有识别到主章节。请确认章标题使用二级标题，例如：## 1 Introduction"
        )
...
def promote_chapter_headings(
    ...
    # 只把原章标题由二级提升为一级；标题文字完全保留原文，
    # 不根据内部排序号重新拼接“第 X 章”或数字章号。
    output: list[str] = [f"# {original_heading}\n"]
...
def write_home_page(
    ...
    if has_front_matter:
        lines.append("- [前置内容](00-front-matter/)\n")

    for item in results:
        relative = item.markdown_path.parent.relative_to(docs_dir).as_posix()
        lines.append(f"- [{item.original_heading}]({relative}/)\n")
...
def split_book_to_project(
    ...
    for position, start in enumerate(chapter_starts):
        ...
        segment_lines = promote_chapter_headings(
            lines=segment_lines,
            original_heading=start.original_heading,
        )
    ...
    total_missing = len(front_missing) + sum(
